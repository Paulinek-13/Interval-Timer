...
STATE IntroLoop(App* app)
{
	...
	ALLEGRO_SAMPLE* sample_intro = al_load_sample("res//samples//sample_intro.ogg");
	if(!sample_intro)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load sample_intro.ogg", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}

	ALLEGRO_BITMAP* bitmap_timer_logo = al_load_bitmap("res//bitmaps//bitmap_timer_logo.png");
	if(!bitmap_timer_logo)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load bitmap_timer_logo.png", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	...
	if(!error)
	{
		...
		// note: variables for drawing timer logo
		float logo_a = 0;
		...
		// note: variables for drawing text 'click to continue'
		...
		while(!end_intro_loop)
		{
			...
			switch(event.type)
			{
				case ALLEGRO_EVENT_DISPLAY_CLOSE: { next_state = STATE::END; end_intro_loop = true; }break;
				...
				case ALLEGRO_EVENT_TIMER:
				{
					...
					if(!text_draw)
					{
						if(logo_a < 1.0) logo_a += 0.01; else logo_a = 1.0;
						if(logo_scale < 1.0) logo_scale += 0.01;
						if(logo_angle < 12.5) logo_angle += 0.1;
						else { logo_angle = 12.56637; text_draw = true; }
					}
					else
					{
						if(logo_py > 200)logo_py--;
						if(logo_scale > 0.5)logo_scale -= 0.02; else logo_scale = 0.5;
					}

					if(text_draw)
					{
						if((text_a == 0) || (text_a == 255))text_add_a = !text_add_a;
						if(text_add_a)text_a += 3; else text_a -= 3;
					}
				}break;
