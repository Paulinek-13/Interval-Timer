...
STATE MainLoop(App *app)
{
	...
	ALLEGRO_SAMPLE *sample_key_down = al_load_sample("resources//samples//key_down.ogg");
	if (!sample_key_down)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load key_down.ogg", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	...
	if (!error)
	{
		...
		while (!end_main_loop)
		{
			...
			{
				// when not typing to input buttons
				if (!input_active)
				{
					for (Input &input : inputs)
					{
						if (input.hovered)
						{
							// start taking keyboard input to activated button
							input_active = &input;
							input.on = false;
							input.str_value = "";
							break;
					...
					{
						// prevent from starting timer with sets set to 0 or time exercise set to 0
						if ((inputs[0].value != 0) && (inputs[2].value != 0))
						{
							...
							app->pop_up.New("set time exercise!", 300, &buttons[4]);
						else if (inputs[2].value <= 0)
							app->pop_up.New("set number of sets!", 300, &buttons[4]);
					}
					...
					else if (buttons[6].hovered)
					{
						...
						if (result == 1)
						{
							...
							{
								inputs[0].ChangeValue(app->time_exercise);
								inputs[1].ChangeValue(app->time_rest);
								inputs[2].ChangeValue(app->sets);
							}
						}
					}
					...
					if (app->audio_stream)
					{
						if (buttons[0].hovered)
						{
							...
							al_set_audio_stream_playing(app->audio_stream, buttons[0].on);
						}
						else if (buttons[1].hovered)
						{
							if (buttons[1].on)
							{
								...
								app->pop_up.New("max volume!", 400, &buttons[1]);
						}
						else if (buttons[2].hovered)
						{
							if (buttons[2].on)
							{
								...
								app->pop_up.New("min volume!", 400, &buttons[2]);
						}
					}
					else if (buttons[0].hovered)
						app->pop_up.New("choose music to turn on/off!", 400, &buttons[0]);
					else if (buttons[1].hovered)
						app->pop_up.New("choose music to turn up!", 400, &buttons[1]);
					else if (buttons[2].hovered)
						app->pop_up.New("choose music to turn down!", 400, &buttons[2]);
				}
				else
				{
					// as it enter key was typed
			...
			{
				// take keyboard input only when one input button is activated
				if (input_active)
				{
				...
				else
				{
					...
					if (input_hovered->on && result)
						input_hovered->hovered = true;
					if (result)
						al_play_sample(sample_key_down, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, 0);
				}
			}
			break;
