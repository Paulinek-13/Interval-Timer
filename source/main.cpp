...
int main()
{
	...
	app.bitmap_icon = al_load_bitmap("resources//icon.png");
	if (!app.bitmap_icon)
	{
	...
	app.font = al_load_font("resources//fonts//kenny_pixel_square.ttf", 32, 0);
	if (!app.font)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load kenny_pixel_square.ttf", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	al_set_new_bitmap_flags(ALLEGRO_MIN_LINEAR | ALLEGRO_MAG_LINEAR);
