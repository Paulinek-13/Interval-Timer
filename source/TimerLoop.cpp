...
STATE TimerLoop(App *app)
{
	...
	ALLEGRO_SAMPLE *sample_exercise = al_load_sample("resources//samples//exercise.ogg");
	if (!sample_exercise)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load exercise.ogg", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	ALLEGRO_SAMPLE *sample_finished = al_load_sample("resources//samples//finished.ogg");
	if (!sample_finished)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load finished.ogg", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	ALLEGRO_SAMPLE *sample_rest = al_load_sample("resources//samples//rest.ogg");
	if (!sample_rest)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load rest.ogg", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	...
	if (!error)
	{
		...
		while (!end_timer_loop)
		{
			...
			case ALLEGRO_EVENT_MOUSE_BUTTON_DOWN:
			{
				...
				// buttons that affect sounds
				else if (buttons[7].hovered)
				{
					buttons[7].on = !buttons[7].on;
					app->play_sounds = buttons[7].on;
				}
				else if (buttons[8].hovered)
				{
					if (buttons[8].on)
					{
						...
						if (app->volume_sounds >= MAX_VOLUME)
						{
							app->volume_sounds = MAX_VOLUME;
							buttons[8].on = false;
						}
						buttons[9].on = true;
					}
					else
						app->pop_up.New("max volume!", 400, &buttons[8]);
				}
				else if (buttons[9].hovered)
				{
					...
					{
						app->volume_sounds -= 0.5;
						if (app->volume_sounds <= MIN_VOLUME)
						{
							app->volume_sounds = MIN_VOLUME;
							buttons[9].on = false;
						}
						buttons[8].on = true;
					}
					else
						app->pop_up.New("min volume!", 400, &buttons[9]);
				}
				...
				// buttons that affect audio stream
				if (app->audio_stream)
				{
					if (buttons[3].hovered)
					{
						buttons[3].on = !buttons[3].on;
						al_set_audio_stream_playing(app->audio_stream, buttons[3].on);
					}
					else if (buttons[4].hovered)
					{
						if (buttons[4].on)
						{
							...
							app->pop_up.New("max volume!", 400, &buttons[4]);
					}
					else if (buttons[5].hovered)
					{
						...
						{
							app->volume -= 0.5;
							if (app->volume <= MIN_VOLUME)
							{
								app->volume = MIN_VOLUME;
								buttons[5].on = false;
							}
							buttons[4].on = true;
							al_set_audio_stream_gain(app->audio_stream, app->volume);
							...
							app->pop_up.New("min volume!", 400, &buttons[5]);
					}
				}
				else if (buttons[3].hovered)
					app->pop_up.New("choose music to turn on/off!", 400, &buttons[3]);
				else if (buttons[4].hovered)
					app->pop_up.New("choose music to turn up!", 400, &buttons[4]);
				else if (buttons[5].hovered)
					app->pop_up.New("choose music to turn down!", 400, &buttons[5]);
			}
			break;
			...
			if (draw_timer_loop && al_is_event_queue_empty(event_queue))
			{
				...
				// show time and done sets
				if ((timer_state != TIMER_STATE::FINISHED) && (timer_state != TIMER_STATE::STOPPED))
				{
					// show time to elapse as: interval.to_elapse+1 so we have 1-20 instead of 0-19
