...
STATE InfoLoop(App* app)
{
	...
	ALLEGRO_BITMAP* bitmap_PAULINEK_logo = al_load_bitmap("res//bitmaps//bitmap_PAULINEK_logo.png");
	if(!bitmap_PAULINEK_logo)
	{
		al_show_native_message_box(0, "ERROR", "Failed to load bitmap_PAULINEK_logo.png", "", 0, ALLEGRO_MESSAGEBOX_ERROR);
		error = true;
	}
	...
	if(!error)
	{
		...
		while(!end_info_loop)
		{
			...
			switch(event.type)
			{
				case ALLEGRO_EVENT_DISPLAY_CLOSE: { next_state = STATE::END; end_info_loop = true; }break;
			...
			if(draw_info_loop && al_is_event_queue_empty(event_queue))
			{
				...
				// note: draw credits
				{
					DrawText(app->font, 700, 30, 0, DISPLAY_WIDTH / 2, 30, 1, al_map_rgb(0, 0, 0),
							 "Used library : Allegro 5 (https ://liballeg.org/)");
					DrawText(app->font, 700, 30, 0, DISPLAY_WIDTH / 2, 60, 1, al_map_rgb(0, 0, 0),
							 "Sound effects obtained from www.zapsplat.com");
					DrawText(app->font, 700, 30, 0, DISPLAY_WIDTH / 2, 90, 1, al_map_rgb(0, 0, 0),
							 "Icon made by Gregor Cresnar from www.flaticon.com");
					DrawText(app->font, 700, 30, 0, DISPLAY_WIDTH / 2, 120, 1, al_map_rgb(0, 0, 0),
							 "Font Pixel Square downloaded from kenny.com");
				}
